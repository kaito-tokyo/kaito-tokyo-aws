...
import { GroupMembership } from "kaito-tokyo-aws-l2constructs";
...
export class AdminGroupMembershipsStack extends cdk.Stack {
	constructor(scope: Construct, id: string, props: AdminGroupMembershipsStackProps) {
		super(scope, id, props);

		const { identityStoreId, importedControlTowerIdentities, users } = props;

		const umireonUserId = findUserIdByEmail(users, "umireon@kaito.tokyo");
