...
import { aws_identitystore as identitystore, aws_sso as sso } from "aws-cdk-lib";
...
export class ImportedControlTowerIdentitiesStack extends cdk.Stack {
	...
	constructor(scope: Construct, id: string, props: ImportedControlTowerIdentitiesStackProps) {
		...
		this.administratorAccess = new sso.CfnPermissionSet(this, "AdministratorAccess", {
		...
		this.organizationFullAccess = new sso.CfnPermissionSet(this, "OrganizationFullAccess", {
			instanceArn: this.ssoInstance.attrInstanceArn,
		...
		this.serviceCatalogAdminFullAccess = new sso.CfnPermissionSet(
			...
			"ServiceCatalogAdminFullAccess",
			{
				...
				managedPolicies: ["arn:aws:iam::aws:policy/AWSServiceCatalogAdminFullAccess"],
				sessionDuration: "PT1H"
			}
		);
		this.serviceCatalogAdminFullAccess.applyRemovalPolicy(cdk.RemovalPolicy.RETAIN);
		...
		this.serviceCatalogEndUserAccess = new sso.CfnPermissionSet(
			...
			"ServiceCatalogEndUserAccess",
			{
				...
				managedPolicies: ["arn:aws:iam::aws:policy/AWSServiceCatalogEndUserFullAccess"],
				sessionDuration: "PT1H"
			}
		);
		this.serviceCatalogEndUserAccess.applyRemovalPolicy(cdk.RemovalPolicy.RETAIN);
	}
}
